        ...
        stage('Create Environment for Kubernetes and Docker') {
              steps {
                    ...
                    script {
                        ...
                        private_ip_deployed = sh ( script: 'cd terraform;terraform output kuber_node_aws_instance_private_ip', returnStdout: true).trim()
                        node_one = sh ( script: 'cd terraform;terraform output kuber_master_aws_instance_private_ip', returnStdout: true).trim()
                    }
                 }
              }
        }
         ...
         stage('install kubernetes master') {
            environment {
              ...
              steps  {
                ...
                echo "awsserver ansible_port=22 ansible_host=${SERVER_DEPLOYED}" > inventory_hosts
                echo "kuber_node_1 ansible_port=2222 ansible_host=localhost" >> inventory_hosts
              ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -vv  -i inventory_hosts --user ubuntu --extra-vars "workspace=${WORKSPACE} target=awsserver" ${WORKSPACE}/playbooks/install-kubernetes-master-playbook.yml
              '''
                 }
              }
        }
