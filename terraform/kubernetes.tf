...
resource "aws_instance" "kuber_master_ec2_instance" {
  ...
  subnet_id = aws_subnet.kubernetes_public_subnet.id
  vpc_security_group_ids = [aws_security_group.allow_ssh.id,aws_security_group.kubernetes_default_sg.id,aws_security_group.allow_kubernetes_master.id,aws_security_group.allow_squid.id,aws_security_group.allow_api.id]
